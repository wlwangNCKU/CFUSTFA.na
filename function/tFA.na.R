...
#tFA.na-EM
tFA.na.EM = function(Y.na, q, init.para, eta=0.005, tol, max.iter, per)
{
  begin = proc.time()[1]
  n = nrow(Y.na)
  p = ncol(Y.na)
  ...
  logli.fn = expression(sum(lgamma((nu+poj)/2)) - n*lgamma(nu/2) -
                          sum(log(det.o))/2 - po*log(pi*nu)/2 - sum((nu+poj)*log(1+Delta.o/nu)/2))
  Qnu = function(nu, la) -(nu*log(nu/2)/2-lgamma(nu/2)+la*nu/2)
  
  logli.old = eval(logli.fn)
  cat(paste("tFA.na.EM is running...", sep = ""), "\n")
  ...
  repeat
  {
    ...
    tau = (nu + poj) / (nu + Delta.o)
    kappa = digamma((nu+poj)/2) - log((nu+Delta.o)/2)
    la = mean(kappa - tau)
    ga = S.inv %*% B
    om = diag(1,q) - t(ga) %*% B
    u.j=t(ga)%*%(Y.h - mu)
    ...
    mu=colSums(tau* t(Y.h -(B%*%u.j)))/sum(tau)
    B= ((Y.h - mu)%*%(tau*t(u.j))+ sum.om%*%ga) %*% solve( u.j%*%(tau*t(u.j))+t(ga)%*%sum.om%*%ga + (n*om) )
    s2=diag((((Y.h - mu)-(B%*%u.j))%*% (tau*t((Y.h - mu)-(B%*%u.j)))  )   + sum.om -  
              (sum.om%*%ga%*%t(B))-(B%*%t(ga)%*%sum.om)+ (B%*%t(ga)%*%sum.om%*%ga%*%t(B)) +  ((B%*%om%*%t(B))*n)     )/n
    nu = nlminb(nu, Qnu, lower=0.00001,upper=Inf,la=la)$par
    s2[s2<eta] = eta
    ...
    det.o = uni.det.o[na.class]
    logli.new = eval(logli.fn)
    diff = logli.new - logli.old
    if (iter%%per == 0) cat("iter =", iter, "\tlogli =", logli.new, "\tdiff =", diff, "\tnu=", nu, "\n")
  ...
  cat(paste(rep("=",60),sep="",collapse=""),"\n")
  cat("iter =", iter, "\tlogli =", logli.new, "\tdiff =", diff, "\tnu=", nu, "\n")
  cat('tFA.na.EM takes', end - begin, 'seconds.\n')
