cfustna.se.new = function(Y.na, q, s, para.est)
{
  n = nrow(Y.na)
  p = ncol(Y.na)
  mu=para.est$mu
  ...
  for(j in 1:n) 
  {
    ...
    Laj = -a_nu * eta[j,] %*% t(rep(1,s)) + H[,,j] - a_nu^2 * tau[j] * La %*% rep(1,s) %*% t(rep(1,s)) + a_nu * La %*% rep(1,s) %*% t(gamma1[j,]) + a_nu * La %*% gamma1[j,] %*% t(rep(1,s)) - La %*% gamma2[,,j]
    sla = rbind(sla,c(Laj)) #n*p
